...
class Arguments:
    def __init__(self, agent, env=None, env_func=None, env_args=None):
        ...
        self.worker_num = (
            16  # rollout workers number pre GPU (adjust it to get high GPU usage) #
        )
        self.thread_num = (
            16  # cpu_num for pytorch, `torch.set_num_threads(self.num_threads)`
        )
        ...
        self.eval_times1 = 2**3  # number of times that get episode return in first # 2**2
        self.eval_times2 = 2**3  # number of times that get episode return in second # 2**4
