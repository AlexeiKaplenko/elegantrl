...
class NnReshapeT(nn.Module):
    def __init__(self, *args):
        super().__init__()
        self.args = args

    def forward(self, x):
        return x.view((x.size(0),) + self.args).transpose(1, -1)
...
class ActorPPO(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim):
        ...
        state_dim = state_dim // DIVIDER

        ## Working
        self.net = nn.Sequential(
                            NnReshapeT(state_dim, DIVIDER),
                            nn.Linear(state_dim, mid_dim), nn.ReLU(),
                            
                            TransformerEncoderLayer(mid_dim, 8, mid_dim, DROPOUT, batch_first=True),
                            TransformerEncoderLayer(mid_dim, 8, mid_dim, DROPOUT, batch_first=True),

                            nn.Flatten(-2),
                            
                            #!!!
                            nn.Linear(DIVIDER * mid_dim, mid_dim), nn.Hardswish(),
                            nn.Linear(mid_dim, int(mid_dim // 4)), nn.Hardswish(),
                            nn.Linear(int(mid_dim // 4), action_dim), 

                            # nn.Linear(DIVIDER * mid_dim, int(mid_dim // 4)), nn.Hardswish(),
                            # nn.Linear(int(mid_dim // 4), action_dim), 
                            )
