...
class Evaluator:  # [ElegantRL.2022.01.01]
    def __init__(self, cwd, agent_id, eval_env, args):
        ...
        self.eval_times1 = max(1, int(args.eval_times))
        self.eval_times2 = max(1, int(args.eval_times))
        self.target_return = args.target_return
        self.if_overwrite = False
    ...
    def evaluate_save_and_plot(
        ...
        if time.time() - self.eval_time < self.eval_gap:
            ...
            if if_save_agent:  # save checkpoint with highest episode return
                ...
                act_name = 'actor' if self.if_overwrite else f'act_target_ObjC_{log_tuple[0]:04.4f}_ObjA_{log_tuple[1]:04.4f}'
                act_path = f"{self.cwd}/{act_name}_{self.r_max:09.3f}.pth"
...
def get_episode_return_and_step(
    env, act
) -> Tuple[float, int]:  # [ElegantRL.2022.01.01]
...
def save_learning_curve(
    ...
    fig_name="plot_learning_curve.png",
):
    if recorder is None:
    ...
    plt.savefig(f"{cwd}/{fig_name}", dpi = 300)
    plt.close(
        "all"
